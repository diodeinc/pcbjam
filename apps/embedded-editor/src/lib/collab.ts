...
export function diffItemSnapshots(
  ...
  return rootOnly(
    deltaToItemsWire(docDelta(previous, next), next.items),
    previous
  );
}
...
export function materializeRootDiff(
  ...
  if (from instanceof Y.Doc) {
    ...
    return rootOnly(
      deltaToItemsWire({ added, updated, removed }, desired.doc.items),
      previous.doc
    );
  }
  return rootOnly(
    deltaToItemsWire(docDelta(from, desired.doc), desired.doc.items),
    from
  );
}
