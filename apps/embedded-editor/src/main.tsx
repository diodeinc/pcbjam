...
function App({ host }: { host: HostConnection | null }) {
  ...
  useEffect(() => {
    ...
    host.onOpen = (value) => {
      ...
      if (opened) return;
      opened = true; setOpen(value); setTheme(value.theme);
    };
    host.onPolicy = readOnly => setOpen(value => value ? {...value, readOnly} : value);
    host.onTheme = setTheme;
  ...
  return <main>
    <header><strong>PCBJam</strong><span>Board editor</span>
      {!host && <><label className="file-button">Open board<input aria-label="Open board" type="file" accept=".kicad_pcb" onChange={e => { void load(e.target.files?.[0]); e.target.value = ""; }} /></label>
      ...
      <a href="./licenses.html" target="_blank" rel="noreferrer">GPL · Source</a>
    </header>
    {error && <div role="alert">{error}</div>}
    {host ? open ? <SandboxKicadPane boardPath={open.filename} host={host} readOnly={open.readOnly} /> : <section className="welcome">Waiting for host…</section> :
